    ...
    <div class="p-4 lg:p-8 bg-gray-50 min-h-screen">
        ...
        <div class="flex flex-col xl:flex-row xl:items-end justify-between gap-6 mb-8">
            <div>
                ...
                <p class="text-gray-500 mt-1">
                    @if($date)
                        Đang xem ngày: <span class="font-semibold text-blue-600">{{ \Carbon\Carbon::parse($date)->format('d/m/Y') }}</span>
                    @elseif($month)
                        Đang xem tháng: <span class="font-semibold text-blue-600">{{ \Carbon\Carbon::parse($month.'-01')->format('m/Y') }}</span>
                    @else
                        Tất cả dữ liệu
                    @endif
                </p>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-5 mb-10">
            <div class="bg-white rounded-3xl p-6 shadow shadow-gray-100 border border-gray-100">
                ...
                <h3 class="text-4xl font-bold text-gray-900 mt-3">{{ $summary['total'] ?? 0 }}</h3>
            </div>
            <div class="bg-white rounded-3xl p-6 shadow shadow-gray-100 border border-gray-100">
                <p class="text-sm font-medium text-emerald-600">Đúng giờ</p>
                <h3 class="text-4xl font-bold text-emerald-600 mt-3">{{ $summary['present'] ?? 0 }}</h3>
            </div>
            <div class="bg-white rounded-3xl p-6 shadow shadow-gray-100 border border-gray-100">
                <p class="text-sm font-medium text-orange-600">Đi muộn</p>
                <h3 class="text-4xl font-bold text-orange-600 mt-3">{{ $summary['late'] ?? 0 }}</h3>
            </div>
            <div class="bg-white rounded-3xl p-6 shadow shadow-gray-100 border border-gray-100">
                <p class="text-sm font-medium text-sky-600">Về sớm</p>
                <h3 class="text-4xl font-bold text-sky-600 mt-3">{{ $summary['early_leave'] ?? 0 }}</h3>
            </div>
            <div class="bg-white rounded-3xl p-6 shadow shadow-gray-100 border border-gray-100">
                <p class="text-sm font-medium text-red-600">Chưa hoàn thành</p>
                <h3 class="text-4xl font-bold text-red-600 mt-3">{{ $summary['incomplete'] ?? 0 }}</h3>
            </div>
        </div>
        ...
        <div class="bg-white rounded-3xl shadow shadow-gray-100 border border-gray-100 overflow-hidden mb-8">
            <div class="p-6 lg:p-8 border-b border-gray-100 bg-gray-50">
                <form method="GET" action="{{ route('admin.attendance.index') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 items-end">

                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Theo Tháng</label>
                        <input type="month" name="month" value="{{ $month ?? '' }}"
                               class="w-full h-12 px-4 rounded-2xl border border-gray-200 focus:border-blue-500 transition">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="user_id" class="w-full h-12 px-4 rounded-2xl border border-gray-200 focus:border-blue-500 transition">
                    ...
                    <div>
                        ...
                        <select name="status" class="w-full h-12 px-4 rounded-2xl border border-gray-200 focus:border-blue-500 transition">
                    ...
                    <div class="flex gap-3">
                        <button type="submit" class="flex-1 h-12 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-2xl transition">
                            Lọc
                        </button>
                        <a href="{{ route('admin.attendance.index') }}"
                           class="h-12 px-6 border border-gray-300 hover:bg-gray-50 rounded-2xl flex items-center justify-center text-gray-600 font-medium">
                            Xóa lọc
                        </a>
                    </div>
                </form>
            ...
            <div class="px-6 py-4 bg-white border-b flex gap-2 flex-wrap">
                <a href="{{ route('admin.attendance.index', ['month' => date('Y-m')]) }}"
                   class="px-4 py-2 text-sm bg-white border border-gray-200 hover:border-blue-300 rounded-xl transition">
                    Tháng này
                </a>
                <a href="{{ route('admin.attendance.index', ['month' => date('Y-m', strtotime('-1 month'))]) }}"
                   class="px-4 py-2 text-sm bg-white border border-gray-200 hover:border-blue-300 rounded-xl transition">
                    Tháng trước
                </a>
            </div>
                    ...
                    @forelse($attendances as $schedule)
                        @php
                            ...
                            $shiftStart = \Carbon\Carbon::parse(
                                $schedule->work_date->format('Y-m-d') . ' ' . $schedule->shift->start_time
                            );

                            $shiftEnd = \Carbon\Carbon::parse(
                                $schedule->work_date->format('Y-m-d') . ' ' . $schedule->shift->end_time
                            );
                            ...
                            if (!$record) {
                                ...
                                if ($record->check_in_time && !$record->check_out_time
                                    && $now->gt($shiftEnd)
                                    && !in_array($record->status, ['forgot_checkout', 'incomplete'])) {
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-6">
                                ...
                                <div class="text-xs text-gray-500">
                                    ...
                                    @if($schedule->user?->department) • {{ $schedule->user->department->name }} @endif
                                </div>
                            </td>
                            <td class="px-6 py-6">
                                <span class="font-medium text-gray-700">{{ \Carbon\Carbon::parse($schedule->work_date)->format('d/m/Y') }}</span>
                            </td>
                            <td class="px-6 py-6">
                                @if($schedule->shift)
                                    <div class="text-sm">
                                        <span class="font-medium">{{ $schedule->shift->name }}</span><br>
                                        <span class="text-gray-500">{{ \Carbon\Carbon::parse($schedule->shift->start_time)->format('H:i') }} - {{ \Carbon\Carbon::parse($schedule->shift->end_time)->format('H:i') }}</span>
                                    </div>
                                @else
                            ...
                            <td class="px-6 py-6 font-mono">{{ $record?->check_in_time ? \Carbon\Carbon::parse($record->check_in_time)->format('H:i') : '--:--' }}</td>

                            <td class="px-6 py-6 font-mono">{{ $record?->check_out_time ? \Carbon\Carbon::parse($record->check_out_time)->format('H:i') : '--:--' }}</td>
                            ...
                            <td class="px-6 py-6 text-sm">
                                @if($record?->late_minutes)<div class="text-orange-600">Muộn: {{ $record->late_minutes }}p</div>@endif
                                @if($record?->early_leave_minutes)<div class="text-sky-600">Sớm: {{ $record->early_leave_minutes }}p</div>@endif
                            </td>
                            ...
                            <td class="px-6 py-6 text-center">
                                ...
                                <span class="inline-block px-5 py-2 text-xs font-semibold rounded-3xl"
                                      style="{{ $badgeStyle }}">
            ...
            <div class="px-7 py-5 bg-gray-50 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                @if ($attendances->hasPages())
                    <nav class="flex items-center space-x-1" role="navigation" aria-label="Pagination Navigation">
                        ...
                        @else
                            <a href="{{ $attendances->previousPageUrl() }}" class="px-3 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-50 transition-colors">
                                &laquo; Trước
                            </a>
                        @endif
                        ...
                        @if ($attendances->hasMorePages())
                            <a href="{{ $attendances->nextPageUrl() }}" class="px-3 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-50 transition-colors">
                                Sau &raquo;
                            </a>
                        @else
                            <span class="px-3 py-2 bg-gray-100 text-gray-400 rounded-lg text-sm border border-gray-200 cursor-not-allowed select-none">
